...
"use client";
import React, { useEffect } from "react";
import NextImage from "next/image"; // ✅ saxitaan
import { X, ChevronLeft, ChevronRight } from "lucide-react";
import { GalleryItem } from "./Gellery";
...
const ImageModal: React.FC<ImageModalProps> = ({
  ...
  return (
    <div
      ...
      onClick={handleBackdropClick}
    >
      <div className="relative max-w-6xl w-full max-h-[90vh] min-h-[500px] flex items-center justify-center">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 w-10 h-10 bg-black/50 hover:bg-black/70 rounded-full flex items-center justify-center text-white transition-all duration-200 hover:scale-110"
        ...
        <div className="w-full h-full flex items-center justify-center min-h-[400px]">
          {image.type === "video" ? (
            ...
            <div className="relative w-full h-full max-w-4xl max-h-[80vh] min-h-[400px] flex items-center justify-center">
              <NextImage
                src={image.src}
                alt={image.title}
                width={800}
